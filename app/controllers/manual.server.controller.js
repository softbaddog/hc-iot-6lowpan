	...
	switch (req.path) {
		case '/manual/embedapp':
			...
			courseJSON = '[{' +
				...
				'"description": {"zh":"在无垠的银河系中，有5艘宇宙飞船因通信故障与地球失去了联系，由于相距遥远，无法直接联系到地球，只能通过行星多跳（Mesh）方式才能与地球取的联系，做为其中一架飞船指挥官的您，请尽快尝试恢复通信，离开这一危险区域。<hr><p>任务一：快速掌握华为nStack协议栈（socket网络编程）使用方法。</p><p>任务二：根据操作指导，使用IAR编译工程，填入关键api，调用socket接口恢复网络通信。</p><p>任务三：将工程编译成二进制固件包，烧录到飞船通信模块中，当观察到设备接入到Mesh网络，即完成最终任务。</p><img src=/img/space_plane.jpg width=450 height=300>", "en":"In the vast Milky Way galaxy, 5 spacecrafts lose contact with the earth as a result of communication failures. Long distances prevent direct contact, and a multi-hop mesh network provides the only potential solution. As a commander, you must restore communications, and escape from the danger zone.<hr><p>Task 1: Master the usage of Huawei nStack interconnection middleware as quickly as possible.</p><p>Task 2: Following the instruction and using an IAR compiling project, fill in key APIs, and call the socket interface to restore network communications.</p><p>Task 3: Compile the project into a binary firmware package, and burn it into the communication module of the spacecraft. Once the device is connected to the Mesh network, the final task is complete.</p><img src=/img/space_plane.jpg width=450 height=300>"}, ' +
				'"content": {"zh":"nStack IoT组件介绍","en":"Introduction to nStack IoT Component"}, ' +
				'"detail": {"zh":"nStack IoT组件是Huawei LiteOS在短距无线嵌入式物联网领域的通信协议栈，其中集成了IEEE 802.15.4，6LoWPAN和RPL等多个协议栈。在深入了解nStack IoT组件之前，我们先来看看基于6LoWPAN的IP协议栈与传统IP协议栈的差别。<br/><img src=/img/ip_vs_6lowpan.png width=600 heigth=250><br>可以看到，6LoWPAN与传统TCP/IP分层非常类似，由于nStack IoT组件具有良好的c接口，只要有过使用BSD socket套接字网络编程经验，便可在非常短的时间迅速掌握nStack IoT组件的使用。<p><strong>IEEE 802.15.4</strong><br/>IEEE802.15.4定义了低速率无线个域网的物理层和媒体访问控制协议，属于IEEE 802.15工作组。运行在2.4GHz或Sub-GHz的ISM频段上，IEEE 802.15.4标准的低功耗、低成本和低速率的特点使其在嵌入式设备通信领域得到广泛的应用。与WIFI不同的是，IEEE 802.15.4能够支持设备间多跳通信模式，通过形成一个Mesh网络极大地扩展通信的覆盖范围。</p><p><strong>基于IPv6的低速无线个域网</strong><br/>由于IEEE 802.15.4的MTU（最大报文段长度）非常小，未经处理的IPv6协议直接运行在这种协议上非常的不切实际，因为无法携带更多有用的数据信息。6LoWPAN使用无状态IPv6/UDP头部字段压缩，显著减小了IPv6的头部大小（从40bytes到2/3字节），使得IPv6能完美得运行在低速率无线个域网。此外，6LoWPAN还定义了邻居发现机制的优化和分片重组的方法。</p><p><strong>低功耗有损网络路由协议</strong><br/>读作“Ripple”，IETF为低功耗有损网络定义的距离向量路由协议，RPL可以基于不同的路由度量和限制条件创建不同的网络路由拓扑，华为nStack通过RPL路由算法优化，能够使大规模节点组成的网络在短时间内快速收敛，当节点发生故障后，整个网络快速自愈和修复。</p><p><strong>CoAP</strong><br/>CoAP是一个轻量级RESTful架构的应用层协议，与HTTP协议类似，但是更加适用于物联网中的资源受限节点。</p><p><strong>EAP-PANA</strong><br/>EAP-PANA是多跳低功耗有损网络下的密钥管理和认证方案，EAP是一个在无线网络中广泛应用的认证协议（例如WIFI无线链路层使用的认证协议），PANA使得EAP能够运行在IP网络中。</p><p><strong>DTLS</strong><br/>基于UDP的安全协议，DTLS假设连接可能是不可靠的，或者数据包到达服务器是会产生乱序。DTLS通常与CoAP协议结合使用，加密应用层数据保障传输端到端安全，例如端设备和云端服务器传输数据时。</p>","en":"The nStack IoT component is the interconnection middleware of Huawei LiteOS in the wireless embedded IoT LR-WPAN domain. It integrates protocol stacks such as IEEE 802.15.4, 6LoWPAN, and RPL. <br/><img src=/img/ip_vs_6lowpan.png width=600 heigth=250><p><strong>IEEE 802.15.4</strong><br/>A low rate wireless personal area network specification which defines the physical layer and media access control layer in ISM bands and in certain sub-GHz bands. IEEE 802.15.4 standard offers a low-cost, low-power, low-speed ubiquitous communication between devices. Unlike WiFi, it enables a multi-hop communication mode between devices enabling coverage of longer distances by forming a mesh network. </p><p><strong>6LoWPAN (IPv6 over Low power Wireless Personal Area Network)</strong><br/>6LoWPAN enables IPv6 connectivity over low rate WPANs by using stateless compression of IPv6/UDP header. Since the MTUs (maximum transmission units) of IEEE 802.15.4 networks are very limited，thus carrying IPv6 header without compression is impractical. 6LoWPAN adaptation layer is introduced to reduce this header size. 6LoWPAN also defines other primitives such as optimized neighbor discovery operations and fragmentation/reassembly at its layer.</p><p><strong>RPL (Routing Protocols for LLNs)</strong><br/>RPL (pronounced ripple) is distance vector proactive routing protocol designed especially for low power and lossy networks with interconnections characterized by high loss rates, low data rates and instability of the links. RPL allows creating routing topologies based on different metrics and constraints. nStack optimizes certain aspects of RPL enabling faster network convergence times and faster maintenance operations such as network repairs.</p><p><strong>CoAP</strong><br/>CoAP is a RESTful application protocol designed to easily translate HTTP for simplified integration with the web with the constrained nodes and networks. </p><p><strong></strong></p>"}, ' +
				'"file": ""' +
				'},{' +
