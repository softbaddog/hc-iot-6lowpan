...
HCC_LIGHTS.DataInterface=function(lineMana_){
    ...
    socket.on('connect', function () {
        socket.on('nodeChanged', function (msg) {
            ...
            scope.updateTopo();
            scope.onlineUpdate();
            scope.lineMana.controlsBrightness(msg.name,(msg.level/20));
        });
        socket.on('brightnessChanged', function (msg) {
            scope.lineMana.controlsBrightness(msg.name,(msg.level/20));
        });
    ...
    this.updateTopo=function(){
        ...
        /* local data test
        if(testEnabled){
            this.lineMana.loadTopoData("data/connectData.json");
        }else{
            this.lineMana.loadTopoData("data/connectData2.json");
        }
        */
        //this.lineMana.loadTopoData("data/connectData.json");
    };
    ...
    this.onlineUpdate=function(){
        ...
        /* local data test
        if(testEnabled) {
            this.lineMana.requestOnlineData("data/onlineData.json");
        }else{
            this.lineMana.requestOnlineData("data/onlineData2.json");
        }*/
        //this.lineMana.requestOnlineData("data/onlineData.json");
    };
    ...
    this.getBrightNess=function(){
        ...
        var ary_=HCC_LIGHTS.star.starGroups;
        ...
        if(group_){
            ...
            for(i_=0;i_<l_;i_++){
                ...
                for(subi_=0;subi_<subl_;subi_++){          
                    ...
                    var item_={name:" ",brightness:" "};
                    item_.name=sub_[subi_].starName;
                    item_.brightness=sub_[subi_].brightness;
