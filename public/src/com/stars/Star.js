...
HCC_LIGHTS.Star2=function(radius_,scene_,textures_,x_,y_,z_){
    this.setLightsScale=function(){};
    this.intensity=0;
    ...
    var glowShader={
        ...
        uniforms: {
            'texture': { type: 't', value: 0, texture: HCC_LIGHTS.textureLoader.load(textures_) }
        },
        vertexShader: [
        ...
        fragmentShader: [
            ...
            'vec3 atmosphere = vec3( 0.2, 0.6, 1.0 ) * pow( intensity, 5.0 );',
    ...
    var starGeo = new THREE.SphereGeometry(radius_, 32, 32);
    var glowMaterial=new THREE.ShaderMaterial(glowShader);
    var star=new THREE.Mesh(starGeo,glowMaterial);
    star.position.set(x_,y_,z_)
    scene_.add(star);

    var starlandMaterial=new THREE.MeshLambertMaterial({
        depthTest:true,
        //color: 0x666666,
        color: 0x223366,
        ...
        transparent: true,
        //opacity:0.2,
        map:HCC_LIGHTS.textureLoader.load(textures_)
    })
    var starland=new THREE.Mesh(starGeo,starlandMaterial);
    star.add(starland);
    ...
    var sphereAtmoMesh = new THREE.Mesh(starGeo, earthAtmoMat);
    star.add(sphereAtmoMesh);

    this.sprites=star;
}
HCC_LIGHTS.Star2.prototype={
    amp:0,
}
HCC_LIGHTS.Earth=function(scene_) {//????
    ...
    */
    ...
    earthCloudsMat.bumpMap=textureLoader.load("textures/earth/earth_clouds_2048.jpg");
    ...
    var cloudsGeo=new THREE.SphereGeometry(100.01, 100, 100);
...
HCC_LIGHTS.StarSprites=function(color_){//???
    ...
    this.setLightsScale=function(val_){
        ...
        val_+=this.amp;//????
        this.brightness=parseInt(val_/20);
        this.brightness>5?this.brightness=5:this.brightness;
